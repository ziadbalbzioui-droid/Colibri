    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        <div className="bg-white rounded-xl border border-border p-4 text-center">
          <p className="text-xs line-through text-slate-400 mt-0.5">{(totalAnnee * 2).toLocaleString("fr-FR", { maximumFractionDigits: 2 })} € facturé</p>
          <p className="text-xl font-bold text-green-600">{totalAnnee.toLocaleString("fr-FR", { maximumFractionDigits: 2 })} €</p>
          <p className="text-xs text-muted-foreground mt-0.5">Total payé (année)</p>
        </div>
        <div className="bg-white rounded-xl border border-border p-4 text-center">
          {enAttente > 0 ? (
            <>
              <p className="text-xs line-through text-slate-400 mt-0.5">{(enAttente * 2).toLocaleString("fr-FR", { maximumFractionDigits: 2 })} € facturé</p>
              <p className="text-xl font-bold text-amber-600">{enAttente.toLocaleString("fr-FR", { maximumFractionDigits: 2 })} €</p>
            </>
          ) : (
            ...
            {factures.map((f: FactureRow) => (
              <div key={f.id} className="px-5 py-4 flex items-center gap-4">
                ...
                <div className="text-right shrink-0 hidden sm:block">
                  <p className="text-xs line-through text-slate-400">{(f.montant_brut * 2).toLocaleString("fr-FR", { maximumFractionDigits: 2 })} €</p>
              ...
              <div className="px-6 py-5 space-y-5">
                <div className="bg-muted/50 rounded-xl p-4 space-y-2.5">
                  ...
                  <div className="flex justify-between text-sm">
                    ...
                    <span className="line-through text-slate-400">{(payModal.montant_brut * 2).toLocaleString("fr-FR", { maximumFractionDigits: 2 })} €</span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span className="text-muted-foreground">Crédit d'impôt (50%)</span>
                    <span className="text-green-600 font-medium">−{payModal.montant_brut.toLocaleString("fr-FR", { maximumFractionDigits: 2 })} €</span>
                  </div>
                  <div className="flex justify-between text-sm border-t border-border pt-2.5">
                    <span className="font-medium">Votre part</span>
                    <span className="font-bold text-xl text-green-600">{payModal.montant_brut.toLocaleString("fr-FR", { maximumFractionDigits: 2 })} €</span>
                  </div>
                </div>
                ...
                >
                  ...
                  Confirmer le paiement — {payModal.montant_brut.toLocaleString("fr-FR", { maximumFractionDigits: 2 })} €
                </button>
              </div>
            )}
          </div>
