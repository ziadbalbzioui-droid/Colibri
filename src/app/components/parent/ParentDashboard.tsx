...
import { Calendar, Clock, CreditCard, ChevronRight, CheckCircle, AlertCircle, BookOpen, Loader2 } from "lucide-react";
    ...
    return (
      <div className="max-w-lg mx-auto text-center py-20">
        ...
        <p className="text-muted-foreground text-sm">
          Votre compte parent n'est pas encore lié à un élève. Contactez votre professeur pour qu'il fasse le lien.
        </p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm mt-1">Tableau de bord de suivi pour {child.nom}</p>
      </div>
      ...
      <div className="bg-gradient-to-r from-primary to-[#1565C0] rounded-2xl p-6 text-white">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="text-right hidden sm:block">
            ...
            <span className="inline-block bg-white/20 text-xs px-2.5 py-0.5 rounded-full">{child.matiere}</span>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        {[
          ...
          },
          {
            ...
            sub: enAttente > 0 ? `Après crédit d'impôt : ${Math.round(enAttente * 0.5)} €` : "Tout est payé ✓",
            icon: CreditCard,
            color: enAttente > 0 ? "bg-amber-50 text-amber-600" : "bg-green-50 text-green-600",
          },
        ].map((stat) => (
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div className="bg-white rounded-xl border border-border overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-border">
            ...
            <Link to="/parent/cours" className="text-xs text-primary hover:underline flex items-center gap-0.5">
              Voir tout <ChevronRight className="w-3.5 h-3.5" />
            </Link>
          </div>
          <div className="divide-y divide-border">
            {prochaines.length === 0 ? (
              <p className="px-5 py-6 text-sm text-muted-foreground text-center">Aucune séance planifiée</p>
            ) : prochaines.map((c) => (
        ...
        <div className="bg-white rounded-xl border border-border overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-border">
            ...
            <Link to="/parent/factures" className="text-xs text-primary hover:underline flex items-center gap-0.5">
              Voir tout <ChevronRight className="w-3.5 h-3.5" />
            </Link>
          </div>
          <div className="divide-y divide-border">
            {dernieresFactures.length === 0 ? (
              <p className="px-5 py-6 text-sm text-muted-foreground text-center">Aucune facture</p>
              ...
              <div key={f.id} className="px-5 py-3.5 flex items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {f.montant_brut}€ brut · {Math.round(f.montant_brut * 0.5)}€ après CI
                  </p>
                </div>
                {f.statut === "en attente" ? (
                  <Link to="/parent/factures" className="text-xs bg-primary text-white px-3 py-1.5 rounded-lg hover:bg-primary/90 font-medium shrink-0">
                    Payer
                  </Link>
                ) : (
                  <span className="text-xs bg-green-50 text-green-700 px-2.5 py-1.5 rounded-lg shrink-0">Payée</span>
                )}
              </div>
            ))}
          </div>
