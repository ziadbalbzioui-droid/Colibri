...
export function CasierJudiciaire() {
  ...
  const [uploaded, setUploaded] = useState(!!profile?.casier_judiciaire_url);
  ...
  async function handleUpload() {
    ...
    try {
      ...
      const { error: profileError } = await updateProfile({
        casier_judiciaire_url: urlData.publicUrl,
      } as any);
      ...
      <div className="bg-white rounded-xl border border-border p-6 space-y-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="font-semibold text-lg">Extrait de casier judiciaire</h2>
          </div>
          <p className="text-muted-foreground text-sm">
            Déposez votre extrait de casier judiciaire (bulletin n°3) datant de moins de 3 mois.
            Ce document est obligatoire pour être autorisé à exercer auprès de mineurs.
          </p>
        </div>

        <div className="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 text-xs text-blue-700 space-y-1">
          <p className="font-semibold">Comment obtenir votre extrait ?</p>
          <p>· Rendez-vous sur <span className="font-medium">casier.justice.fr</span></p>
          <p>· Demandez le bulletin n°3 (usage privé)</p>
          <p>· Formats acceptés : JPG, PNG, PDF — max 5 Mo</p>
        </div>
        ...
        {uploaded ? (
          <div className="flex flex-col items-center gap-3 py-6">
            ...
            <p className="text-sm text-muted-foreground">Votre casier judiciaire est en cours de vérification par notre équipe.</p>
            <button
